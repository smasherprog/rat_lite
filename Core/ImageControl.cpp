	...
	namespace Remote_Access_Library {
		namespace UI {
			...
			public:
				...
				Utilities::Point _OriginalImageSize;//helper to avoid locking where its not needed

				std::shared_ptr<Utilities::Image> _ScaledImage;
				Utilities::Point _ScaledImageSize;//helper to avoid locking where its not needed
				...
				Utilities::Point get_UnscaledImageSize() const {
				...
				void set_ScreenImage(std::shared_ptr<Utilities::Image>& img) {
					auto tmp = Utilities::Image::CreateImage(img->Height(), img->Width(), img->data(), 4);
					{
						std::lock_guard<std::mutex> lock(_ImageLock);
						_OriginalImageSize = Utilities::Point(img->Width(), img->Height());
						_OriginalImage = img;
						_ScaledImageSize = Utilities::Point(img->Width(), img->Height());
						_ScaledImage = tmp;
					}
				}
				...
				void set_ImageDifference(Utilities::Point& pos, const std::shared_ptr<Utilities::Image>& img) {
					...
					if (_OriginalImage) {
						...
						auto difdst_rect = Utilities::Rect(pos, (int)img->Height(), (int)img->Width());
						auto difsrc_rect = Utilities::Rect(Utilities::Point(0, 0), (int)img->Height(), (int)img->Width());

						if (_ScaledImage) {
							if (is_ImageScaled() && _OriginalImageSize.Y != _ScaledImageSize.Y) {//rescale the incoming image image

								auto resampledimg = Utilities::Image::Resize(img, _ScaleFactor);
								...
								{
									std::lock_guard<std::mutex> lock(_ImageLock);
									Utilities::Image::Copy(*img, difsrc_rect, *_OriginalImage, difdst_rect);//keep original in sync
				...
				bool Update(int& width, int& height) {
					...
					if (is_ImageScaled()) {
						...
						if (psize != _ScaledImageSize.Y) {//rescale the image
							{
								std::lock_guard<std::mutex> lock(_ImageLock);
								_ScaledImage = Utilities::Image::Resize(_OriginalImage, psize, static_cast<int>(_ScaleFactor*_OriginalImage->Width()));
								_ScaledImageSize = Utilities::Point(_ScaledImage->Width(), _ScaledImage->Height());
							}
						...
						if (_ScaledImageSize.X != _OriginalImageSize.X || _ScaledImageSize.Y != _OriginalImageSize.Y) {
							{
								std::lock_guard<std::mutex> lock(_ImageLock);
								_ScaledImage = Utilities::Image::CreateImage(_OriginalImage->Height(), _OriginalImage->Width(), _OriginalImage->data(), 4);
								_ScaledImageSize = Utilities::Point(_ScaledImage->Width(), _ScaledImage->Height());
							}
							width = _ScaledImage->Width();
							height = _ScaledImage->Height();
			...
			public:
				...
				void handlemouse(int e, int button, Input::Mouse::Press press, int x, int y) {
					if (_ScreenImageDriver.is_ImageScaled()) {
				...
				virtual int handle(int e) override {
					...
					switch (e) {
					case FL_PUSH:
						handlemouse(e, Fl::event_button(), Input::Mouse::Press::DOWN, Fl::event_x() + _ScreenImageInfo.get_Left(), Fl::event_y() + _ScreenImageInfo.get_Top());
						return 1;
					case FL_RELEASE:
						handlemouse(e, Fl::event_button(), Input::Mouse::Press::UP, Fl::event_x() + _ScreenImageInfo.get_Left(), Fl::event_y() + _ScreenImageInfo.get_Top());
						break;
					case FL_ENTER:
						return 1;
					case FL_DRAG:
					case FL_MOUSEWHEEL:
					case FL_MOVE:
						handlemouse(e, Input::Mouse::Events::NO_EVENTDATA, Input::Mouse::Press::NO_PRESS_DATA, Fl::event_x() + _ScreenImageInfo.get_Left(), Fl::event_y() + _ScreenImageInfo.get_Top());
						break;
					case FL_KEYDOWN:
						_ScreenImageInfo.OnKey(e, Input::Keyboard::Press::DOWN);
						return 1;
					case FL_KEYUP:
						_ScreenImageInfo.OnKey(e, Input::Keyboard::Press::UP);
						return 1;
					case FL_FOCUS:
						return 1;
					case FL_PASTE:
...
SL::Remote_Access_Library::UI::ImageControl::ImageControl(int X, int Y, int W, int H, const char * title, ScreenImageInfo&& info) {
...
void SL::Remote_Access_Library::UI::ImageControl::set_ImageDifference(Utilities::Point & pos, const std::shared_ptr<Utilities::Image>& img)
{
	return _ImageControlImpl->_ScreenImageDriver.set_ImageDifference(pos, img);
}

void SL::Remote_Access_Library::UI::ImageControl::set_MouseImage(std::shared_ptr<Utilities::Image>& img)
{
	return _ImageControlImpl->_ScreenImageDriver.set_MouseImage(img);
}

void SL::Remote_Access_Library::UI::ImageControl::set_MousePosition(Utilities::Point * pos)
{
