...
#define RAIIHANDLE(handle) std::unique_ptr<std::remove_pointer<HANDLE>::type, decltype(&::CloseHandle)>(handle, &::CloseHandle)
			...
			std::shared_ptr<Utilities::Image> CaptureMouseImage() {
				...
				if(GetCursorInfo(&cursorInfo) == FALSE) return Utilities::Image::CreateImage(0, 0);

				ICONINFO ii = { 0 };
				if (GetIconInfo(cursorInfo.hCursor, &ii) == FALSE) return Utilities::Image::CreateImage(0, 0);
				...
				if (DrawIcon(capturedc.get(), 0, 0, cursorInfo.hCursor) == FALSE) return Utilities::Image::CreateImage(0, 0);
				BITMAP bm;
				int height = 0;
...
SL::Remote_Access_Library::Capturing::Mouse::Mouse(std::function<void(std::shared_ptr<Utilities::Image>)> img_func, std::function<void(Utilities::Point)> pos_func, int img_dely, int pos_dely)
{
