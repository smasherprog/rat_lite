...
{
	_ConnectWindow->HideWithEffect(wxSHOW_EFFECT_BLEND);
	...
	auto child = new MainWindow(_MainWindow, "RDP Window", host, "6000", [this]() {
		_ConnectWindow->ShowWithEffect(wxSHOW_EFFECT_BLEND);
	});
	sizer->Add(child, 1, wxALL | wxEXPAND);
...
bool SL::Remote_Access_Library::UI::Viewer::OnInit()
{
	...
	_ConnectWindow = new ConnectWindow(
		...
		[this]() {
			if (_MainWindow != nullptr) _MainWindow->Close();
			if (_ConnectWindow != nullptr) _ConnectWindow->Close();
		});
	_ConnectWindow->Show();
	return true;
}
