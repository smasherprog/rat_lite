	...
	namespace RAT {
		...
		class ClientNetworkDriverImpl {
			...
			void ScreenImage(const IWebSocket& socket, const char* data, size_t len) {
				...
				if (tjDecompress2(jpegDecompressor, src, static_cast<unsigned long>(len - sizeof(Rect)), (unsigned char*)outputbuffer.data(), outwidth, 0, outheight, TJPF_RGBX, 2048 | TJFLAG_NOREALLOC) == -1) {
					SL_RAT_LOG(Logging_Levels::ERROR_log_level, tjGetErrorStr());
				}
