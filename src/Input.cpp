...
namespace SL {
	namespace RAT {
		...
		void SimulateMouseEvent(const MouseEvent & m)
		{
			...
			SL_RAT_LOG(Logging_Levels::INFO_log_level, "SetMouseEvent EventData:" << m.EventData << " ScrollDelta: " << m.ScrollDelta << " PressData: " << m.PressData);
			...
			switch (m.EventData) {
			case Events::LEFT:
				if (m.PressData == Press::UP) input.mi.dwFlags |= MOUSEEVENTF_LEFTUP;
				else if (m.PressData == Press::DOWN) input.mi.dwFlags |= MOUSEEVENTF_LEFTDOWN;
				else input.mi.dwFlags |= MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_LEFTUP;
				break;
			case Events::MIDDLE:
				if (m.PressData == Press::UP) input.mi.dwFlags |= MOUSEEVENTF_MIDDLEUP;
				else if (m.PressData == Press::DOWN) input.mi.dwFlags |= MOUSEEVENTF_MIDDLEDOWN;
				else input.mi.dwFlags |= MOUSEEVENTF_MIDDLEDOWN | MOUSEEVENTF_MIDDLEUP;
				break;
			case Events::RIGHT:
				if (m.PressData == Press::UP) input.mi.dwFlags |= MOUSEEVENTF_RIGHTUP;
				else if (m.PressData == Press::DOWN) input.mi.dwFlags |= MOUSEEVENTF_RIGHTDOWN;
			...
			if (m.PressData == Press::DBLCLICK) SendInput(1, &input, sizeof(input));
#elif __APPLE__
			...
			switch (m.EventData) {
			case Events::LEFT:
				if (m.PressData == Press::UP) XTestFakeButtonEvent(display, Button1, False, CurrentTime);
				else if (m.PressData == Press::DOWN) XTestFakeButtonEvent(display, Button1, True, CurrentTime);
				else {//double click
				...
				break;
			case Events::MIDDLE:
				if (m.PressData == Press::UP) XTestFakeButtonEvent(display, Button2, False, CurrentTime);
				else if (m.PressData == Press::DOWN) XTestFakeButtonEvent(display, Button2, True, CurrentTime);
				else {//double click
				...
				break;
			case Events::RIGHT:
				if (m.PressData == Press::UP) XTestFakeButtonEvent(display, Button3, False, CurrentTime);
				else if (m.PressData == Press::DOWN) XTestFakeButtonEvent(display, Button3, True, CurrentTime);
				else {//double click
...
#define SL_DIVIDE         0xf06F
		...
		{
			SL_RAT_LOG(Logging_Levels::INFO_log_level, "Received SetKeyEvent Key:" << ev.Key << " SpecialKey: " << ev.SpecialKey << " PressData: " << ev.PressData);
			...
			if (ev.Key & 0xf000) {//special key, needs to be mapped to platform specific code
				...
				SL_RAT_LOG(Logging_Levels::Debug_log_level, "Recevied a key event which is outside of the asci char set");
				return;
			}
#endif

			if (ev.Key == 0) {//no mapping available
				SL_RAT_LOG(Logging_Levels::Debug_log_level, "No Mapping Available for key");
				return;
			}
			...
			input.ki.dwFlags = ev.PressData == Press::UP ? KEYEVENTF_KEYUP : 0;
			input.ki.wVk = ev.Key;
			SendInput(1, &input, sizeof(input));
			...
			XTestFakeKeyEvent(display, keycode, ev.PressData == Press::DOWN ? True : False, CurrentTime);
			//XSync(display, True);
