			...
			public:
				...
				Asio_Context() : work(io_service) {
					io_servicethread = std::thread([this]() {
						SL_RAT_LOG(Logging_Levels::INFO_log_level, "Starting io_service Factory");
						...
						SL_RAT_LOG(Logging_Levels::INFO_log_level, "stopping io_service Factory");
					});
				}
				~Asio_Context()
				{
	...
	namespace Remote_Access_Library {
		...
		namespace Network {
			...
			public:
				...
				ListinerImpl(IBaseNetworkDriver* netevent, std::shared_ptr<Asio_Context> asiocontext, unsigned short port, std::shared_ptr<Network::Server_Config> config) :
					_acceptor(asiocontext->io_service, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), port)),
					_config(config),
